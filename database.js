...
exports.dbProductInfo = (req, res) => {
  ...
  var db = openDB();
  var statement = db.prepare("SELECT Products.product_id, Products.name, Manufactures.name as maker, Products.image, Products.price FROM Products, Manufactures WHERE Products.product_id = Manufactures.product_id AND Products.product_id = ?");
